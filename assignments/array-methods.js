...
// Problem 1
// Vida Tydd has let us know they won't be albe to run in this event. Remove them from the list of runners.
const newRunners = runners.filter(
  runner => runner.last_name !== "Tydd" && runner.first_name !== "Vida"
);
console.log(newRunners);

// Problem 2
// We need to know how many runners have size Large shirts so we can order them from a different printing company
const numberOfLarge = runners.filter(runner => runner.shirt_size === "L")
  .length;
console.log(`Large Shirts: ${numberOfLarge}`);

// Problem 3
// We need a list of email addresses so we can send all the runners the newsletter
const runnerEmails = runners.map(runner => runner.email);
